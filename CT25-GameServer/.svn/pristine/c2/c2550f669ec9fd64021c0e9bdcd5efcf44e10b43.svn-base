	...
	class LMStartTask implements Runnable
	{
		...
		private void announce(long time)
		{
			if (time >= 3600 && time % 3600 == 0)
			{
				...
				{
					Announcements.getInstance().announceToAll("LM Event: " + (time / 60 / 60) + " hour(s) until registration is closed!");
				}
			...
			else if (time >= 60)
			{
				if (LMEvent.isParticipating())
				{
					Announcements.getInstance().announceToAll("LM Event: " + (time / 60) + " minute(s) until registration is closed!");
				}
				else if (LMEvent.isStarted())
				{
			...
			else
			{
				if (LMEvent.isParticipating())
				{
					Announcements.getInstance().announceToAll("LM Event: " + time + " second(s) until registration is closed!");
				}
				else if (LMEvent.isStarted())
				{
