...
public class Hitman
{
	...
	private static String[] SQL_OFFLINE = { "select charId, char_name from characters where char_name = ?", "select charId, char_name from characters where charId = ?" };
	...
	public void onDeath(L2PcInstance assassin, L2PcInstance target)
	{
		if (_targets.containsKey(target.getObjectId()))
		{
			...
			if (!Config.HITMAN_SAME_TEAM)
				...
				{
					assassin.sendChatMessage(assassin.getObjectId(), Say2.TELL, "Agent", "You is not considered a Hitman, do not accept betrayal.");
					assassin.sendChatMessage(assassin.getObjectId(), Say2.TELL, "Agent", "You never received the reward for it. (Same Clan/Ally)");
					return;
				}
			...
			target.sendChatMessage(target.getObjectId(), Say2.TELL, assassin.getName(), "You were on the target list. I received a reward for murdering you.");
			
			if (client != null)
			{
				client.sendChatMessage(client.getObjectId(), Say2.TELL, assassin.getName(), "Your assassination request to kill " + target.getName() + " has been fulfilled.");
				client.setHitmanTarget(0);
			}
			
			assassin.sendChatMessage(assassin.getObjectId(), Say2.TELL, (client != null ? client.getName() : "Hitman"), "The murder of " + target.getName() + " you will receive your reward.");
	...
	public void onEnterWorld(L2PcInstance activeChar)
	{
		...
		if (activeChar.getHitmanTarget() > 0)
		{
			...
			{
				activeChar.sendChatMessage(activeChar.getObjectId(), Say2.TELL, "Hitman Info", "Your target has been eliminated. Have a nice day.");
				activeChar.setHitmanTarget(0);
			}
			else
				activeChar.sendChatMessage(activeChar.getObjectId(), Say2.TELL, "Hitman Info", "Your target is still at large.");
		}
	}
	...
	public void putHitOn(L2PcInstance client, String playerName, Long bounty, Integer itemId)
	{
		...
		{
			client.sendChatMessage(client.getObjectId(), Say2.TELL, "Agent", "You are already a client here, you can place a request only for a single player.");
			return;
		}
		else if (client.getInventory().getInventoryItemCount(itemId, -1) < bounty)
		{
			client.sendMessage("It has mirrors the amount " + ItemTable.getInstance().getTemplate(itemId).getName() + ".");
			return;
		}
		else if (player == null && CharNameTable.getInstance().doesCharNameExist(playerName))
		{
			...
			{
				client.sendChatMessage(client.getObjectId(), Say2.TELL, "Agent", "There is already a hit on that player.");
				return;
			}
			...
			if (Config.HITMAN_ANNOUNCE)
				Announcements.getInstance().announceToAll(client.getName() + " paid " + 
						(bounty > 999 ? f.format(bounty) : bounty) + " " +
		...
		else if (player != null && CharNameTable.getInstance().doesCharNameExist(playerName))
		{
			...
			player.sendChatMessage(client.getObjectId(), Say2.TELL, "Agent", "There is a hit on you.");
			...
			client.sendChatMessage(client.getObjectId(), Say2.TELL, "Agent", "Player name invalid. The user you added does not exist.");
	}
	...
	public void cancelAssasination(String name, L2PcInstance client)
	{
		...
		else if (target == null && CharNameTable.getInstance().doesCharNameExist(name))
		{
			...
			else if (pta.getClientId() == client.getObjectId())
			{
				...
				client.sendChatMessage(client.getObjectId(), Say2.TELL, "Agent", "You are not the actual owner of that target!");
		}
		else if (target != null && CharNameTable.getInstance().doesCharNameExist(name))
		{
			PlayerToAssasinate pta = _targets.get(target.getObjectId());
			...
			client.sendChatMessage(client.getObjectId(), Say2.TELL, "Agent", "Player name invalid. The user u added dose not exist.");
	}
