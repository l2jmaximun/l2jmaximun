...
public class FinalEmperialTomb extends Quest
{
    ...
    private synchronized void enterInstance(L2PcInstance player)
    {
        ...
        if (checkConditions(player))
        {
            ...
            {
            	if (!player.isGM() && !player.destroyItemByItemId("Frintezza", 8073, 1, null, true))
            	{
	                for (L2PcInstance partyMember : party.getCommandChannel().getMembers())
	                {
			...
			try
            {
                switch(_taskId)
                {
		            ...
		            case 6:
                        ...
                        break;
                    case 7:
                        for (L2Character pc : world._Zone.getCharactersInside().values())
    ...
    {
        for (L2Character cha : world._Zone.getCharactersInside().values())
        {
			cha.abortAttack();
			cha.abortCast();
	...
	public FinalEmperialTomb(int id, String name, String descr)
	{
		super(id, name, descr);
