...
public class SubClassCertification extends Quest
{
    	...
    	private static final int[] NPC = 
	{
    	...
    	private static final int[] WARRIORCLASSES = {3,88,2,89,46,48,113,114,55,117,56,118,127,131,128,129,132,133};
    	private static final int[] ROGUECLASSES = {9,92,24,102,37,109,130,134,8,93,23,101,36,108};
    	...
    	private static final int[] WIZARDCLASSES = {12,94,13,95,27,103,40,110};
    	private static final int[] HEALERCLASSES = {16,97,30,105,43,112};
    	private static final int[] ENCHANTERCLASSES = {17,98,21,100,34,107,51,115,52,116,135,136};
    	...
    	private static final int[] TRANSFORMITEMS = 
	{
    	...
    	private boolean isCorrectMaster(L2Npc npc, L2PcInstance player)
    	{
        	Race currentRace = CharTemplateTable.getInstance().getTemplate(player.getClassId()).race;

        	if ((((L2VillageMasterInstance)npc).getVillageMasterRace() == Race.Human) && (currentRace == Race.Human || currentRace == Race.Elf) && 
			((L2VillageMasterInstance)npc).getVillageMasterTeachType() == ClassType.Fighter && !player.isMageClass())
            		return true;
        	else if ((((L2VillageMasterInstance)npc).getVillageMasterRace() == Race.Human) && (currentRace == Race.Human || currentRace == Race.Elf) && 
			((L2VillageMasterInstance)npc).getVillageMasterTeachType() == ClassType.Mystic && ArrayUtils.isIntInArray(player.getClassId().getId(), WIZARDCLASSES))
            		return true;
        	else if ((((L2VillageMasterInstance)npc).getVillageMasterRace() == Race.Human) && (currentRace == Race.Human || currentRace == Race.Elf) && 
			((L2VillageMasterInstance)npc).getVillageMasterTeachType() == ClassType.Mystic && ArrayUtils.isIntInArray(player.getClassId().getId(), SUMMONERCLASSES))
            		return true;
        	else if ((((L2VillageMasterInstance)npc).getVillageMasterRace() == Race.Human) && (currentRace == Race.Human || currentRace == Race.Elf) && 
			((L2VillageMasterInstance)npc).getVillageMasterTeachType() == ClassType.Priest && ArrayUtils.isIntInArray(player.getClassId().getId(), HEALERCLASSES))
            		return true;
        	else if ((((L2VillageMasterInstance)npc).getVillageMasterRace() == Race.Human) && (currentRace == Race.Human || currentRace == Race.Elf) && 
			((L2VillageMasterInstance)npc).getVillageMasterTeachType() == ClassType.Priest && ArrayUtils.isIntInArray(player.getClassId().getId(), ENCHANTERCLASSES) && player.isMageClass())
            		return true;			
            		...
            		if (type.equals("Emergent"))
            		{
               			...
               			if (event.equals("lvl65Emergent"))
               			{
                    			if (isAvailable65.equals("") || isAvailable65.equals("0"))
                    			{
                            		...
                            		String html = "<html><body>Сертификация Навыков Подкласса:<br>Вы еще не готовы получить свой %level% уровень сертификации. Трудитесь упорно и возвращайтесь позже.</body></html>";
